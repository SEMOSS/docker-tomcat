...
ENV TOMCAT_HOME=/home/semoss/apache-tomcat-9.0.26

ENV PATH=$PATH:/home/semoss/apache-maven-3.5.4/bin:$TOMCAT_HOME/bin
...
RUN useradd -ms /bin/bash semoss \
	&& apt-get -y update \
	&& apt-get install -y sudo \
	&& apt-get install -y nano \
	&& echo 'semoss ALL = (ALL) NOPASSWD: ALL' >> /etc/sudoers
...
RUN sudo apt-get update \
	...
	#&& mkdir /home/semoss/apache-maven-3.5.4 \
	#&& mv apache-maven-3.5.4/* /home/semoss/apache-maven-3.5.4/ \
	#&& rm -r apache-maven-3.5.4 \
	&& rm apache-maven-3.5.4-bin.tar.gz \
	&& echo '#!/bin/sh' > $TOMCAT_HOME/bin/start.sh \
	&& echo 'catalina.sh start' >> $TOMCAT_HOME/bin/start.sh \
	&& echo 'tail -f /home/semoss/apache-tomcat-9.0.26/logs/catalina.out' >> $TOMCAT_HOME/bin/start.sh \
	&& echo '#!/bin/sh' > $TOMCAT_HOME/bin/stop.sh \
	&& echo 'shutdown.sh -force' >> $TOMCAT_HOME/bin/stop.sh \
	&& chmod 777 $TOMCAT_HOME/bin/*.sh \
	&& chmod 777 /home/semoss/apache-maven-3.5.4/bin/*.cmd \
	&& sudo apt-get clean all \
	&& chown -R semoss:semoss /home/semoss
